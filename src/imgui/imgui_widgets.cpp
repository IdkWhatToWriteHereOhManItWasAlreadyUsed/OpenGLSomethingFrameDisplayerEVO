                        ...
                        ImS64 size_contents_v, ImDrawFlags draw_rounding_flags)
{
    ...
    const float grab_h_pixels = ImClamp(scrollbar_size_v * ((float)size_visible_v / (float)win_size_v), grab_h_minsize,
                                        scrollbar_size_v);
...
bool ImGui::TextLink(const char* label)
{
    ...
    // FIXME-TEXT: Underline mode // FIXME-DPI
                       ...
                       const void* p_max, const char* format, ImGuiSliderFlags flags)
{
    ...
    if (temp_input_is_active)
    {
        ...
        if ((flags & ImGuiSliderFlags_ClampOnInput) && (p_min != NULL || p_max != NULL))
        {
            ...
            // -1 when *p_min < *p_max, == 0 when *p_min == *p_max
                        ...
                        ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* callback_user_data)
{
    ...
    else if ((init_state && g.ActiveId != id) || init_changed_specs)
    {
        ...
        if (recycle_state && (state->TextLen != buf_len || (state->TextA.Data == NULL || strncmp(
    ...
    else
    {
        ...
        if (is_multiline || (buf_display_end - buf_display) < buf_display_max_length)
        {
            ...
            const ImVec2 draw_scroll = /*state ? ImVec2(state->Scroll.x, 0.0f) :*/ ImVec2(0.0f, 0.0f);
...
bool ImGui::TreeNodeBehavior(ImGuiID id, ImGuiTreeNodeFlags flags, const char* label, const char* label_end)
{
    ...
    const bool span_all_columns_label = (flags & ImGuiTreeNodeFlags_LabelSpanAllColumns) != 0 && (g.CurrentTable !=
        NULL);
    ImRect frame_bb;
    ...
    if (!is_visible)
    {
        if ((flags & ImGuiTreeNodeFlags_DrawLinesToNodes) && (window->DC.TreeRecordsClippedNodesY2Mask & (1 << (window->
            DC.TreeDepth - 1))))
        {
    ...
    for (ImGuiSelectionRequest& req : ms_io->Requests)
    {
        ...
        else if (req.Type == ImGuiSelectionRequestType_SetRange)
        {
            ...
            else
            {
                ...
                for (int idx = (int)req.RangeFirstItem; idx <= (int)req.RangeLastItem; idx++, selection_order += req.
                     RangeDirection)
                    ImGuiSelectionBasicStorage_BatchSetItemSelected(this, GetStorageIdFromIndex(idx), req.Selected,
...
bool ImGui::BeginMenuBar()
{
    ...
    ImRect clip_rect(
        IM_ROUND(bar_rect.Min.x + window->WindowBorderSize * 0.5f), IM_ROUND(bar_rect.Min.y + border_top),
        IM_ROUND(
            ImMax(bar_rect.Min.x, bar_rect.Max.x - ImMax(window->WindowRounding, window->WindowBorderSize * 0.5f))),
                      ...
                      ImGuiWindow* docked_window)
{
    ...
    if (hovered && g.HoveredIdNotActiveTimer > TOOLTIP_DELAY && bb.GetWidth() < tab->ContentWidth)
    {
        ...
        bb.Max.x = bb.Min.x + IM_TRUNC(
            ImLerp(bb.GetWidth(), tab->ContentWidth, ImSaturate((g.HoveredIdNotActiveTimer - 0.40f) * 6.0f)));
    ...
    // Render tab shape
    const bool is_visible = (g.LastItemData.StatusFlags & ImGuiItemStatusFlags_Visible) && !(flags &
                                       ...
                                       bool is_contents_visible, bool* out_just_closed, bool* out_text_clipped)
{
    ...
    const bool unsaved_marker_visible = (flags & ImGuiTabItemFlags_UnsavedDocument) != 0 && (button_pos.x + button_sz <=
        bb.Max.x) && (!close_button_visible || !is_hovered);
