...
struct IMGUI_API ImGuiInputTextState
{
    ...
    const char* TextSrc;
    // == TextA.Data unless read-only, in which case == buf passed to InputText(). Field only set and valid _inside_ the call InputText() call.
    ImVector<char> TextA;
...
enum ImGuiDebugLogFlags_
{
    ...
    ImGuiDebugLogFlags_EventFont | ImGuiDebugLogFlags_EventInputRouting | ImGuiDebugLogFlags_EventDocking |
...
// [renamed/changed in 1.92] The types are not actually compatible but we provide this as a compile-time error report helper.
#endif
...
{
    IM_ASSERT(
        index_idx < ImFontAtlasRectId_IndexMask_ && gen_idx < (ImFontAtlasRectId_GenerationMask_ >>
