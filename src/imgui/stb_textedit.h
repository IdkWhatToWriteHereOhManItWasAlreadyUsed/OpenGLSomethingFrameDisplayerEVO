...
{
    return _idx > 0
               ? (STB_TEXTEDIT_IS_SPACE(STB_TEXTEDIT_GETCHAR(_str, _idx - 1)) && !STB_TEXTEDIT_IS_SPACE(
                   STB_TEXTEDIT_GETCHAR(_str, _idx)))
               : 1;
}
...
static void stb_textedit_discard_redo(StbUndoState* state)
{
    ...
    if (state->redo_point <= k)
    {
        ...
        if (state->undo_rec[k].char_storage >= 0)
        {
            ...
            memmove(state->undo_char + state->redo_char_point, state->undo_char + state->redo_char_point - n,
                    (size_t)((size_t)(STB_TEXTEDIT_UNDOSTATECOUNT - state->redo_char_point) * sizeof(
