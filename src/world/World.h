...
namespace OpenGLSomethingFrameDisplayerEVO
{
    ...
    inline int WorldToChunkCoord(const float coord)
    {
    ...
    public:
        GenericObjectBuffer<GPUReadyChunkGPUAlignedData> m_blocksGraphicalData;
        World();
        explicit World(int seed);
        ~World();

        void Initialise(int videoWidth, int videoHeight);
        void SetLoadingDistance(const int distance) { m_loadingDistance = distance; }
        ...
        std::vector<std::array<robin_hood::unordered_set<GLuint>, static_cast<std::size_t>(GeometryType::Count)>>
        m_idsToDelete;
    ...
    private:
        ...
        void GenerateGPUArraysOfStrip(std::vector<std::vector<uint8_t>>& data, int startX, int endX, int threadIndex) const;
    };
}
