...
public:
    VideoWorld();
    explicit VideoWorld(int seed);
    ~VideoWorld();

    std::vector<std::vector<std::unique_ptr<ChunkInstance>>>& GetLoadedChunks() { return m_loadedChunks; }
    ...
    int GetBuffersCount() const { return m_blocksGraphicalData.size();}
