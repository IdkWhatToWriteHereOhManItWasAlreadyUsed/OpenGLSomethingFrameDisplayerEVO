...
namespace OpenGLSomethingFrameDisplayerEVO
{
    ...
    void BlockMeshGenerator::InitialiseCachesMeshes()
    {
        ...
        for (auto& geometry : geometries)
        {
            ...
            {
                geometryGraphicalData.first.insert(geometryGraphicalData.first.end(), face.vertices.begin(),
                                                   face.vertices.end());
    ...
    )
    {
        ...
        if (geometry->can_cull_faces.empty())
        {
            auto lastIndexOffset = 0;
        ...
        else
        {
            ...
            for (int i = 0; i < 6; i++)
            {
                if (!shouldCullFace[i])
                {
                    blockVertices.insert(blockVertices.end(), geometry->faces[i].vertices.begin(),
                                         geometry->faces[i].vertices.end());

                    for (auto index : geometry->faces[i].indices)
                        blockIndices.push_back(index + lastIndexOffset);
                    lastIndexOffset += geometry->faces[i].vertices.size();

                    addedFacesIndicesInVertexArray.push_back({
                        geometry->faces[i].vertices.size() * addedFaceIndexCounter++, i
                    });
                }
            }
