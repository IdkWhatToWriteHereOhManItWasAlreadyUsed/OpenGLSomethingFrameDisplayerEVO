...
namespace OpenGLSomethingFrameDisplayerEVO
{
        ...
        public:
            StepIterator(const glm::vec3& origin, const glm::vec3& direction)
            {
                ...
                tMax.x = (direction.x > 0)
                             ? (voxelMax.x - origin.x) / direction.x
                             : (direction.x < 0)
                             ? (voxelMin.x - origin.x) / direction.x
                             : std::numeric_limits<float>::infinity();

                tMax.y = (direction.y > 0)
                             ? (voxelMax.y - origin.y) / direction.y
                             : (direction.y < 0)
                             ? (voxelMin.y - origin.y) / direction.y
                             : std::numeric_limits<float>::infinity();

                tMax.z = (direction.z > 0)
                             ? (voxelMax.z - origin.z) / direction.z
                             : (direction.z < 0)
                             ? (voxelMin.z - origin.z) / direction.z
                             : std::numeric_limits<float>::infinity();
    ...
    public:
        static RayCastResult castRay(const glm::vec3& start, const glm::vec3& direction,
                                     const World* world, float maxDistance = 32.0f)
        {
